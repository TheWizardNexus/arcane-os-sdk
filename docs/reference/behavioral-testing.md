...
| Provider-neutral AI runtime and chat activation | Provider/2 registration, closed three-role configuration, startup barriers, independent LLM/STT/TTS load/unload/status, cancellation, speech mute, sticky state, selected-unloaded activation request/cancellation/error events, and absence of silent provider fallback are exercised against bounded providers and host callbacks. | Real model/runtime admission remains the selected provider's evidence boundary; a state or activation event does not prove native, cloud, or browser-model availability. |
