...
if (place) console.log(await weather.load(place));`
    },
    {
        ...
        classification:'public-first-party',
        lifecycleSideEffects:'Construction creates or binds one ChatEntity and asynchronously loads existing JSONL history only when requested. send and stream settle pending memory, run one configured chat transaction, project only human-readable records at the ChatEntity DBOPFS boundary, then commit retained provider history atomically. persist:false uses the request and response for that operation and rolls both out of subsequent model context, transcript, memory, and DBOPFS. Existing stored files are never rewritten merely by loading them. stream uses ai.streamRequest when supplied, buffers every observed structural call until the ordered array exactly matches the terminal calls, and otherwise falls back to the configured non-stream fetch/chat path.',
        paramsResults:'new PersistentAIChatSession({ai?|chat?,chatEntity,chatFileName,contextBuilder,loadExisting,maxContextCharacters,maxMessageCharacters,maxMessages,memory=true,request,responseLength,systemPrompt}); ai must expose fetchRequest and may expose streamRequest, while ai and chat are mutually exclusive. static create() and createPersistentAIChatSession() await readiness. ready() waits for initialization and resolves the same session. send accepts user messages or matching tool results; a tool result may supply message, name, and status for its sanitized durable record while content and tool_call_id remain transient provider fields. persist:false returns the completed one-operation response but retains neither side of the turn. stream(input,{onChunk?,onDataChunk?,onDataResult?,onToolCall?}) resolves the same terminal response and exposes validated calls only after the terminal array matches. history() returns complete retained provider-safe context. transcript() returns only role/content/timestamp for retained user and assistant turns and role/content/timestamp plus optional tool name/status for retained tool turns. It never exposes newly written system prompts, reasoning, raw calls, IDs, arguments, raw results, nonpersistent turns, or provider/internal fields; settleMemory() waits for ChatEntity memory work.',
        events:[],
        errors:['AI_CHAT_UNAVAILABLE','AI_CHAT_ABORTED','AI_CHAT_AMBIGUOUS_PROVIDER','AI_CHAT_BUSY','AI_CHAT_INVALID_RESPONSE','AI_CHAT_INVALID_TOOL_CALL','AI_CHAT_INVALID_TOOL_MESSAGE','AI_CHAT_TOOL_MESSAGE_REQUIRED','AI_CHAT_TOOL_RESULT_REQUIRED','AI_CHAT_STREAM_TOOL_CALL_MISMATCH','AI_CHAT_INCOHERENT_PERSISTENCE','AI_CHAT_TRANSACTION_SETTLED','configured chat, ChatEntity, DBOPFS, and memory errors propagate'],
