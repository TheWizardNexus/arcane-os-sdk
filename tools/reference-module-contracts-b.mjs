...
if (place) console.log(await weather.load(place));`
    },
    {
        ...
        paramsResults:'new PersistentAIChatSession({ai?|chat?,chatEntity,chatFileName,contextBuilder,loadExisting,maxContextCharacters,maxMessageCharacters,maxMessages,memory=true,request,responseLength,systemPrompt}); ai must expose fetchRequest and may expose streamRequest, while ai and chat are mutually exclusive. static create() and createPersistentAIChatSession() await readiness. ready() waits for initialization and resolves the same session. send accepts {message:{content,role:user|tool,tool_call_id?,persist=true},request?,response:{persist=message.persist},signal?} or an atomic messages array that supplies one nonblank role=tool result for every pending call ID. stream(input,{onChunk?,onDataChunk?,onDataResult?,onToolCall?}) resolves the same committed terminal response, forwards complete provider data through the data callbacks, exposes each validated structural call only after the complete ordered array matches the terminal calls, and uses the non-stream path when no stream transport exists. history() returns provider-safe live model context and reports malformed persistence with a coded error; transcript() retains complete ChatEntity records with persisted timestamps, legacy ui_hidden metadata, and nonblank unreadable JSONL rows in their original string form for application-owned recovery; settleMemory() waits for ChatEntity memory work.',
        events:[],
