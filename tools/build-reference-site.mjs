...
function reviewedModuleDetails(record){
    if(record.name==='AI.js'){
        return [
            `<h2 id="reviewed-member-contract">Reviewed member contract</h2><p><strong>Constructor:</strong> <code>new AI(llmService='', sttService='', ttsService='', model='', modelTTS='', modelSTT='')</code>. Complete-response inference uses <code>fetch(...)</code> or the object form <code>fetchRequest(options)</code>. Incremental inference uses <code>streamMessage(...)</code> or the object form <code>streamRequest(options)</code>. Both forms share provider implementations; the positional public methods remain supported.</p>`,
            `<p><strong>Provider lifecycle:</strong> the read-only <code>providerRuntime</code> plus <code>configureProviders(config)</code>, <code>transitionAI(llmService, sttService, ttsService, model, modelTTS, modelSTT)</code>, <code>transitionProviders(selections)</code>, <code>startProviders(options)</code>, and <code>setSpeechMuted(muted)</code> keep LLM, STT, and TTS selection explicit. Each transition stops queued audio, unloads all three current roles, and then applies replacement routes; <code>transitionAI()</code> returns aggregate runtime status and <code>transitionProviders()</code> returns the admitted route configuration. Selected TWiN Cloud <code>TWIN</code> LLM, <code>OLLAMA</code> LLM, and local <code>LOCAL_SPEACH</code> audio routes expose truthful capability-only readiness through internal provider/2 adapters; they do not probe, download, or fake a loaded local model. Built-in speech stays on device with Whisper and Kokoro, and non-local speech selections reject.</p>`,
            `<p><strong>Primary methods:</strong> <code>setAI(llmService, sttService, ttsService, model, modelTTS, modelSTT)</code>, <code>streamRequest(options={})</code>, <code>streamMessage(...)</code>, <code>fetchRequest(options={})</code>, <code>fetch(...)</code>, <code>streamTTS(text='', end=false, options={})</code> (third-argument options are available in SDK 0.5.12), <code>finishTTS()</code>, <code>fetchTTS(payload={}, signal=null)</code>, <code>fetchSTT(audioFile, signal=null)</code>, <code>stopAudio()</code>, <code>resumeAudio(audioContext=null, fromUserGesture=true)</code>, and <code>playAudio(audioChunks=[], audioContext, sourceNode=null, audioType=this.audioType, speechJob=null)</code>. Every TTS route automatically removes repeated same formatting marks from only an outbound speech-input copy; applications provide no cleanup option or internal preparation metadata.</p>`,
