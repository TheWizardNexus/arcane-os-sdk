export const referenceModuleContractsA=[
    {
        ...
        example:`// The application supplies this runtime argument; never put its key in source.
...
console.log(value);`
    },
    {
        ...
        paramsResults:'BrowserTestSuite({tests, now, ...metadata}).list() returns complete mutable descriptors. Caller metadata does not limit or time the supplied list. run({context, signal}) returns a complete mutable {status, totals, durationMs, results} summary and supplies assert, skip, signal, and context to each callback.',
        events:['browser-test-suite-start','browser-test-start','browser-test-result','browser-test-suite-complete'],
        errors:['BROWSER_TEST_BUSY','BROWSER_TEST_ABORTED','BROWSER_TEST_TIMEOUT','BROWSER_TEST_SKIP','BROWSER_TEST_ASSERTION','BROWSER_TEST_ERROR','BROWSER_TEST_INVALID_OPTIONS','BROWSER_TEST_INVALID_DESCRIPTOR','BROWSER_TEST_INVALID_CLOCK','BROWSER_TEST_INVALID_RESULT','BROWSER_TEST_CASE_COLLISION'],
...
console.log(status.dataset.tone);`
    },
    {
        ...
        events:['communications-refresh-started','communications-refresh-completed','communications-refresh-partially-completed','communications-refresh-failed','communications-refresh-cancelled'],
...
console.log(window.dbls.get('settings'),window.dbls.count());`
    },
    {
        ...
        paramsResults:"After readyPromise, getTableHandle(tableName) lazily opens or creates one requested table and coalesces concurrent requests for the same physical directory. removeEmptyTable(tableName) resolves the target through getDirectoryHandle(name,{create:false}), then asks OPFS to remove that directory non-recursively and resolves {status:'removed'|'absent'|'not-empty',removed,tableName,directoryName}. It never creates, scans, clears, or recursively removes the target; a same-named file and unexpected platform errors reject without removal. The logical memories table retains its physical memory directory and cache aliases are invalidated when that directory is removed or absent. Existing directories remain discoverable, and DBOPFS supports table and file set, get, batch, filter, count, and delete APIs plus PNG-compressed backup and restore. Storage is rooted at apps/<authoritative-app-id>, and worker fallback handles synchronous OPFS access.",
        events:['dbopfs-ready'],
...
console.log(await picker.select({title:'Choose a workspace'}));`
    },
    {
        ...
        paramsResults:'new DocumentLexicalSearch(records); rank(query,{kinds,tags}) and search(query,{kinds,tags}) return every mutable scored match. documentContextExcerpt(value) returns the complete text and full line range. Public helpers expose normalized text/tokens, index construction, and metadata/body scoring.',
        events:[],
...
console.log(applyDocumentNavigationFilter(binding));`
    },
    {
        name:'Errors.js',
        classification:'public-first-party',
        lifecycleSideEffects:'Browser import auto-installs one global handler, listens for global errors and rejections, assigns occurrence identifiers, persists pending delivery in sessionStorage, may show a developer modal and send bounded mail notifications, and destroy removes listeners, UI, and timers.',
        paramsResults:'normalizeErrorEvent and normalizeRejectionEvent return bounded incident records. Errors(options) supports capture, flush, whenIdle, and destroy with injectable clock, storage, scheduler, mail, and developer UI.',
