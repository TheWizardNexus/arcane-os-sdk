...
test('loader imports only the fixed provider path and returns the canonical Arcane root',async t=>{
    ...
    const loaded=await loadArcanePortableProvider({
        arcaneRoot,
...
test('CLI requires an explicit Arcane root only when pairing the portable provider',async()=>{
    ...
    const loadNativeProvider=async({arcaneRoot})=>{
        loadCalls+=1;
        return {
...
test('CLI leaves other native targets unpaired and loads the provider for portable build',async()=>{
    const stdout=memoryStream();
    const stderr=memoryStream();
    const executions=[];
    let loadCalls=0;
    ...
    assert.equal(loadCalls,0);
    assert.equal(executions[0].options.nativeBuilder,undefined);

    const portableStdout=memoryStream();
    assert.equal(await runCli(
        [
            'build','--target','portable','--arcane-root','C:\\Arcane',
            '--output','ndjson'
        ],
        {stdout:portableStdout,stderr:memoryStream(),execute,loadNativeProvider}
    ),0);
    assert.equal(loadCalls,1);
    assert.equal(executions[1].options.targetRequest.target,'portable');
    assert.equal(executions[1].options.nativeBuilder.protocol,'arcane-native-builder/1');
});
...
test('paired CLI build packages one app and gives the provider only verified release readers',async t=>{
