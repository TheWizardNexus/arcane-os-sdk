...
test('installed npm sources own maps, development serving and portable output',{timeout:120_000},async function installedPackageOnly(){
    ...
    console.log('[installed-package] Nested application PWA import-map.');
    await toolchain.importMap();
    const logicalPaths=[
    ...
    console.log('[installed-package] Nested application serving.');
    ...
    console.log('[installed-package] Nested application packaging.');
    ...
    for(const generated of [
        'arcane-pwa.mjs','arcane-sw.js','arcane-offline.json','arcane.webmanifest'
    ]){
    ...
    const rootWorker=await readFile(path.join(workspaceRoot,'arcane-sw.js'),'utf8');
    assert.ok(rootWorker.includes('"/node_modules/arcane-os/browser-runtime/pwa.mjs"'));
    ...
    for(const endpoint of ['arcane-pwa.mjs','arcane-sw.js','arcane-offline.json','arcane.webmanifest']){
        assert.ok(rootPackaged.files.includes(endpoint),endpoint);
        assert.equal((await stat(path.join(rootPackaged.outputRoot,endpoint))).isFile(),true,endpoint);
    }
    assert.equal(rootPackaged.files.some(function nestedRootFile(file){return file.startsWith('apps/');}),false);
    await assert.rejects(stat(path.join(rootPackaged.outputRoot,'apps')),{code:'ENOENT'});
    ...
    assert.ok(packagedRootWorker.includes('"./node_modules/arcane-os/browser-runtime/pwa.mjs"'));
