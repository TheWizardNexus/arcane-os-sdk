...
- Remove only demonstrably obsolete SDK code. An alternative API, positional
  ...
  explicitly authorizes their retirement. Distinguish required code that was
  mislabeled from genuinely retired support; a rename is not a deletion.
  Preserve recent unrelated work, historical release records, and upstream
  dependency code. Keep maintained references aligned with the actual public API.
