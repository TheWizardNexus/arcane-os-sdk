...
export default class SystemToolRegistry{
    ...
    register(definition={}){
        ...
        if(!is.function(definition.command)&&!is.string(definition.command))throw new TypeError(`System tool ${id} requires a command builder.`);
    ...
    build(id,args=[]){
        ...
        const command=is.function(tool.command)?tool.command(Array.from(args)):tool.command;
        if(!is.string(command)||!command.trim())throw new TypeError(`System tool ${id} produced an empty command.`);
        return command.trim();
    }
}
