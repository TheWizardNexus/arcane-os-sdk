...
This checkout defines the `0.7.1` SDK contract. Applications pin one exact npm
...
```bash
npx arcane-os@0.7.1 new hello-speech --path ./hello-speech --target browser
cd hello-speech
npm install
npm run dev
```
...
```bash
npx arcane-os@0.7.1 new my-app --path ./my-app --target portable --git
cd my-app
npm install
npm run dev
```
...
```bash
npm install --save-dev --save-exact arcane-os@0.7.1
...
Use `npx arcane-os@0.7.1` for the initial bootstrap because it names this npm
...
cd ../local-app
npm install --save-dev --save-exact ../arcane-os-sdk/arcane-os-0.7.1.tgz
npm ci
```
...
the local declaration with the exact `arcane-os@0.7.1` registry package and
...
Version `0.7.1` exposes one browser target and five explicitly paired
